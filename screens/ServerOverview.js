...
import {Card, Provider, Title, Menu, Button, Divider, TextInput, IconButton, Paragraph, DataTable, Checkbox, Colors, Snackbar, Badge} from 'react-native-paper';
...
export default function ServerOverview({route, navigation}) {
  ...
  const renderStatusIcon=(icon)=>{
    if(icon=="running"){
      return <><View style={{alignItems:'center',justifyContent:'center'}}><Icon name="done" size={25} color="#4C9F5A" /></View>      
      <Text style={{fontFamily:'Raleway-Regular',marginLeft: 10,fontSize:12,color:'#4C9F5A'}}>{icon.charAt(0).toUpperCase() +icon.slice(1)}</Text>
      </>;
    }else if(icon=="stopped"){
      return <><View style={{alignItems:'center',justifyContent:'center'}}><Icon name="power-settings-new" size={25} color="#E15241" /></View>      
      <Text style={{fontFamily:'Raleway-Regular',marginLeft: 10,fontSize:12,color:'#E15241'}}>{icon.charAt(0).toUpperCase() +icon.slice(1)}</Text>
      </>;
    }
    else if(icon=="waiting"){
      return <><ActivityIndicator animating={true} size={25} color={Colors.blue400}/>;
      <Text style={{fontFamily:'Raleway-Regular',marginLeft: 10,fontSize:12,color:'#4C9F5A'}}>{icon.charAt(0).toUpperCase() +icon.slice(1)}</Text>
      </>
      ...
      return <>
      <ActivityIndicator animating={true} size={25} color={Colors.blue400} />          
      <Text style={{fontFamily:'Raleway-Regular',marginLeft: 10,fontSize:12,color:'#4C9F5A'}}>{icon.charAt(0).toUpperCase() +icon.slice(1)}</Text>
      </>
    }
   return null;
  ...
  const tabs=[{"label":"Activity","icon":<ActivityIcon width={19} height={19} color="#00a1a1" />,"navigate":"Activity"},
  ...
  return server ? (
    <>
    <View width="100%" height="100%" style={{backgroundColor:'#F4F8F8',paddingHorizontal:'8%',paddingTop:'8%'}}>
      <View style={{display:'flex',flexDirection:'row',alignItems:'center',justifyContent:'space-between'}}>
        <TouchableOpacity onPress={navigation.goBack}><BackIcon height={45} width={50}/></TouchableOpacity>
        <Text style={{color:'#00A1A1',fontFamily:'Raleway-Medium',fontSize:20,textAlign:'center'}}>{(route.params.name.length > 10) ? route.params.name.slice(0, 10-1) + ' ...' : route.params.name}</Text>
        <Menu
              visible={visible}
              onDismiss={closeMenu}
              style={{
              ...
              anchor={
                <TouchableOpacity onPress={openMenu}><MoreIcon height={45} width={50}/></TouchableOpacity>
              }>
              <Menu.Item
                icon="pencil"
                onPress={() => {
...
>
      ...
      <View
        ...
        <TouchableOpacity onPress={()=>setStartModal(true)}
          style={{
            width: '32%',
            alignItems: 'center',            
            flexDirection:'row',
          ...
          <View style={{width:20,height:20,justifyContent:'center',alignItems:'center',backgroundColor:'rgba(76,159,90,0.26)',borderRadius:20/2}}>
          <Icon
            name="play-arrow"
            size={15}
            color="#4C9F5A"
          ...
          <Text style={{marginStart:5,fontFamily:'Raleway-SemiBold',fontSize:10,color:'#4C9F5A'}}>Start server</Text>
        </TouchableOpacity>
        <TouchableOpacity onPress={() => setStopModal(true)}
          style={{
            ...
            <View style={{height:25,justifyContent:'center',alignItems:'center'}}>
            <View style={{width:20,height:20,justifyContent:'center',alignItems:'center',backgroundColor:'rgba(217,75,75,0.26)',borderRadius:20/2}}>
              <Icon
                name="stop"
                size={15}
                color="#E15241"
          ...
          <Text style={{marginStart:5,fontFamily:'Raleway-SemiBold',fontSize:10,color:'#E15241'}}>Stop server</Text>
        </TouchableOpacity>
        <TouchableOpacity onPress={() => setRestartModal(true)}
          style={{
            width: '32%',
            alignItems: 'center',
            flexDirection:'row',
            ...
            marginTop:15,
                        ...
                        <View style={{height:25,justifyContent:'center',alignItems:'center'}}>
            <View style={{width:20,height:20,justifyContent:'center',alignItems:'center',backgroundColor:'rgba(3,155,229,0.26)',borderRadius:20/2}}>
          <Icon
            name="replay"
            size={15}
            color="#449ADF"
          ...
          <Text style={{marginStart:5,fontFamily:'Raleway-SemiBold',fontSize:10,color:'#449ADF'}}>Restart server</Text>
        </TouchableOpacity>
      </View>
      <View>
        <View style={{marginTop:15,backgroundColor:'white',borderRadius:10,            borderBottomRightRadius:0,
            borderBottomLeftRadius:0}}>
          <View style={{display:'flex',padding:15,flexDirection:'row',
          alignItems:'center',justifyContent:'space-between'}}>
            <View>
              <Text style={{fontFamily:'Raleway-Medium',fontSize:12}}>Name</Text>
              <Text style={{fontFamily:'Raleway-Light',fontSize:12}}>{server.name}</Text>
            </View>
          </View>
        </View>
      </View>
      <View>
        <View style={{marginTop:1,backgroundColor:'white',borderRadius:0}}>
          <View style={{display:'flex',padding:15,flexDirection:'row',
          alignItems:'center',justifyContent:'space-between'}}>
{server.aliases?<View>
              <Text style={{fontFamily:'Raleway-Medium',fontSize:12}}>Alias</Text>
              <View style={{flexDirection:'row',alignItems:'center'}}>
                {server.aliases.length==1?
                <>
                <TouchableOpacity style={{flexDirection:'row',alignItems:'center'}} onPress={()=>handleClick(server.aliases[0])}>
                ...
                <Text style={{fontFamily:'Raleway-Light',fontSize:12,marginStart:5}}>{server.aliases[0]}</Text>
                </TouchableOpacity>
                </>
                :<Text style={{fontFamily:'Raleway-Light',fontSize:12}}>{server.aliases[0]}</Text>
                }
              </View>
              { server.aliases.length>1?
              <TouchableOpacity style={{marginTop:10}} onPress={()=>setAliasModal(true)}>
              <Text style={{fontFamily:'Raleway-Regular',fontSize:10,color:'#00A1A1'}}>
                 See {server.aliases.length-1} more</Text>
              </TouchableOpacity>
              :null
              }
            </View>:null}
      ...
      <View>
        <View style={{marginTop:1,backgroundColor:'white',borderRadius:0}}>
          ...
          alignItems:'center',justifyContent:'space-between'}}>
            <View>
              <Text style={{fontFamily:'Raleway-Medium',fontSize:12}}>IPv4</Text>
              <Text style={{fontFamily:'Raleway-Light',fontSize:12}}>{server.ipv4}</Text>
            </View>
          </View>
        </View>
      </View>
      <View>
        <View style={{marginTop:1,backgroundColor:'white',borderRadius:10,borderTopRightRadius:0,
            borderTopLeftRadius:0,}}>
          <View style={{display:'flex',padding:15,flexDirection:'row',
          alignItems:'center',justifyContent:'space-between'}}>
            <View>
              <Text style={{fontFamily:'Raleway-Medium',fontSize:12}}>IPv6</Text>
              <Text style={{fontFamily:'Raleway-Light',fontSize:12}}>{server.ipv6}</Text>
            </View>
          </View>
        </View>
      </View>
      <Text style={{fontFamily:'Raleway-Medium',fontSize:18,marginVertical:10}}>Server info</Text>
      <View>
        <View style={{display:'flex',flexDirection:'row',flexWrap:'wrap',alignItems:'center',justifyContent:'space-between'}}>
          ...
          <View key={item.label} style={{
            ...
            borderRadius:10}}>
            <TouchableOpacity key={item.label} onPress={()=>navigation.navigate(item.navigate)}>
                <View style={{height:125}}>
                    <View style={{display:'flex'}}>
                            <View style={{height:125/2,padding:10,justifyContent:'flex-start',alignItems:'flex-end'}}>
                              <View style={{width:33,height:33,justifyContent:'center',alignItems:'center',backgroundColor:'white',borderRadius:33/2,opacity:0.66}}>{item.icon}</View>
                            </View>
                            <View style={{height:125/2,padding:10,justifyContent:'flex-end',alignItems:'flex-start'}}>
                              <Text style={{fontFamily:'Raleway-Bold',fontSize:18,marginStart:5,marginBottom:5,color:'#00a1a1'}}>{item.label}</Text>
                            </View>
                    </View>
                </View>
            </TouchableOpacity>
    ...
    <Modal
        ...
        <View style={{backgroundColor:'white',padding:30,borderTopStartRadius:10, borderTopEndRadius:10}}>
          <Text style={{fontFamily:'Raleway-Medium',fontSize:18,color:'#00a1a1',marginVertical:10}}>Start server</Text>
          <Text style={{fontFamily:'Raleway-Regular',fontSize:12,color:'#000000',marginVertical:10}}>Do you really want to start this server?</Text>
          <View style={{width:'100%',marginVertical:15,display:'flex',flexDirection:'row',alignItems:'center',justifyContent:'space-between'}}>
            <TouchableOpacity onPress={()=>setStartModal(false)} style={{width:'45%',height:40,backgroundColor:'#D94B4B',borderRadius:4,justifyContent:'center'}}>
                <Text style={{fontFamily:'Raleway-Bold',fontSize:16,color:"#FFFFFF",textAlign:'center'}}>Cancel</Text>
            </TouchableOpacity>
            <TouchableOpacity onPress={()=>startThisServer(route.params.slug)} style={{width:'45%',height:40,backgroundColor:'#4C9F5A',borderRadius:4,justifyContent:'center'}}>
                <Text style={{fontFamily:'Raleway-Bold',fontSize:16,color:"#FFFFFF",textAlign:'center'}}>Start</Text>
            </TouchableOpacity>
          </View>
        </View>
    </Modal>
    ...
    <Modal
        ...
        style={{justifyContent: 'flex-end',margin: 0}}>
        <View style={{backgroundColor:'white',padding:30,borderTopStartRadius:10, borderTopEndRadius:10}}>
          <Text style={{fontFamily:'Raleway-Medium',fontSize:18,color:'#00a1a1',marginVertical:10}}>Stop server</Text>
          <Text style={{fontFamily:'Raleway-Regular',fontSize:12,color:'#000000',marginVertical:10}}>Do you really want to stop this server?</Text>
          <View style={{width:'100%',marginVertical:15,display:'flex',flexDirection:'row',alignItems:'center',justifyContent:'space-between'}}>
            <TouchableOpacity onPress={()=>setStopModal(false)} style={{width:'45%',height:40,backgroundColor:'#00a1a1',borderRadius:4,justifyContent:'center'}}>
                <Text style={{fontFamily:'Raleway-Bold',fontSize:16,color:"#FFFFFF",textAlign:'center'}}>Cancel</Text>
            </TouchableOpacity>
            <TouchableOpacity onPress={()=>stopThisServer(route.params.slug)} style={{width:'45%',height:40,backgroundColor:'#D94B4B',borderRadius:4,justifyContent:'center'}}>
                <Text style={{fontFamily:'Raleway-Bold',fontSize:16,color:"#FFFFFF",textAlign:'center'}}>Stop</Text>
            </TouchableOpacity>
          </View>
        </View>
    </Modal>
    ...
    <Modal
        ...
        style={{justifyContent: 'flex-end',margin: 0}}>
        <View style={{backgroundColor:'white',padding:30,borderTopStartRadius:10, borderTopEndRadius:10}}>
          <Text style={{fontFamily:'Raleway-Medium',fontSize:18,color:'#00a1a1',marginVertical:10}}>Reboot server</Text>
          <Text style={{fontFamily:'Raleway-Regular',fontSize:12,color:'#000000',marginVertical:10}}>Do you really want to reboot this server?</Text>
          <View style={{width:'100%',marginVertical:15,display:'flex',flexDirection:'row',alignItems:'center',justifyContent:'space-between'}}>
            <TouchableOpacity onPress={()=>setRestartModal(false)} style={{width:'45%',height:40,backgroundColor:'#00A1A1',borderRadius:4,justifyContent:'center'}}>
                <Text style={{fontFamily:'Raleway-Bold',fontSize:16,color:"#FFFFFF",textAlign:'center'}}>Cancel</Text>
            </TouchableOpacity>
            <TouchableOpacity onPress={()=>rebootThisServer(route.params.slug)} style={{width:'45%',height:40,backgroundColor:'#449ADF',borderRadius:4,justifyContent:'center'}}>
                <Text style={{fontFamily:'Raleway-Bold',fontSize:16,color:"#FFFFFF",textAlign:'center'}}>Reboot</Text>
            </TouchableOpacity>
          </View>
        </View>
    </Modal>
        ...
        style={{justifyContent: 'flex-end',margin: 0}}>
        <View style={{backgroundColor:'white',padding:30,borderTopStartRadius:10, borderTopEndRadius:10}}>
          <Text style={{fontFamily:'Raleway-Medium',fontSize:18,color:'#00a1a1',marginVertical:10}}>Suspend {route.params.slug}</Text>
          <Text style={{fontFamily:'Raleway-Regular',fontSize:12,color:'#000000',marginVertical:10}}>Do you really want to suspend this server?</Text>
          <View style={{display:'flex',flexDirection:'row',marginVertical:10}}>
            <View style={{backgroundColor:'#03A84E',width:1}}></View>
            <Text style={{fontFamily:'Raleway-Regular',fontSize:12,color:'#000000',marginStart:10}}>This will create a new Snapshot of your server which will be put into cold storage. This enables you to restore this server in the future. Your passwords and settings will not be deleted, and your server shortname (slug) will be reserved. Statistics and monitoring rules will be deleted.</Text>
          </View>
          <View style={{display:'flex',flexDirection:'row',marginVertical:10}}>
            <View style={{backgroundColor:'#03A84E',width:1}}></View>
            <Text style={{fontFamily:'Raleway-Regular',fontSize:12,color:'#000000',marginStart:10}}>You can only have up to 3 suspended servers at any one time. If you want more, click here to view current pricing. You can permanently delete suspended servers in your servers overview. You do not pay for the first 3 suspended servers in your account.</Text>
          </View>
          <View style={{display:'flex',flexDirection:'row',marginVertical:10}}>
            <View style={{backgroundColor:'#f44336',width:1}}></View>
            <Text style={{fontFamily:'Raleway-Regular',fontSize:12,color:'#000000',marginStart:10}}>Warning: Suspending a server means that you are in effect removing it from the Webdock infrastructure and freeing up your IPv4 address for others to use.</Text>
          </View>
          <View style={{width:'100%',marginVertical:15,display:'flex',flexDirection:'row',alignItems:'center',justifyContent:'space-between'}}>
            <TouchableOpacity onPress={()=>setSuspendModal(false)} style={{width:'45%',height:40,backgroundColor:'#00A1A1',borderRadius:4,justifyContent:'center'}}>
                <Text style={{fontFamily:'Raleway-Bold',fontSize:16,color:"#FFFFFF",textAlign:'center'}}>Cancel</Text>
            </TouchableOpacity>
            <TouchableOpacity onPress={()=>suspendThisServer(route.params.slug)} style={{width:'45%',height:40,backgroundColor:'#df0000',borderRadius:4,justifyContent:'center'}}>
                <Text style={{fontFamily:'Raleway-Bold',fontSize:16,color:"#FFFFFF",textAlign:'center'}}>Suspend</Text>
            </TouchableOpacity>
          </View>
        </View>
    </Modal>
