  ...
  const onChangeSearch = query => setSearchQuery(query);
    ...
    React.useLayoutEffect(() => {
        navigation.setOptions({
            headerTitle: () => <TextInput autoFocus placeholderTextColor={'white'}  selectionColor={'white'} 
    ...
    const Item = ({ title,alias,dc,profile,ipv4 }) => (
        <View style={styles.item}>
            ...
            <View style={styles.midinfo}>
              ...
              <Text style={styles.datacenterandprofile}>{<Image
    source={dc==='fi'
        ...
        <FlatList
            ...
            renderItem={({item})=>(
              <TouchableOpacity onPress={()=>navigation.navigate('ServerManagement')}>
                <View>
                  <Item title={item.slug} alias={item.aliases[0]} 
                    dc={item.location} profile={item.profile} ipv4={item.ipv4}/>
                  <Divider/>
                </View>
              </TouchableOpacity>
            )}
